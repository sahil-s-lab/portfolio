...
join wl815963.employee_detail w2 on coalesce(remove_check.channel_id,sa.CHANNEL_ID) = w2.wireless_id
  and w2.effective_start_date <= sa.expiration_date
  and ( w2.effective_end_date >= sa.expiration_date )
...
left join wl815963.employee_attribute w3 on coalesce(remove_check.channel_id,sa.CHANNEL_ID) = w3.wireless_id
  and w3.effective_start_date <= sa.expiration_date
  and ( w3.effective_end_date >= sa.expiration_date )
...
and (sa.expiration_issue_date - sa.effective_issue_date >= 15 or 
  (sa.expiration_issue_date - sa.effective_issue_date < 15 and
...
and rf.UCPNR1_CPNR1_one_time = 0
